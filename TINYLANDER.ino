//   >>>>>  T-I-N-Y  L-A-N-D-E-R v1.0 for ARDUBOY  GPLv3 <<<<
...
#define GAME_FRAME_CONTROL_TLANDER while((currentMillis-MemMillis)<33){currentMillis=millis();}MemMillis=currentMillis
...
void changeSpeed(GAME * game)
{
  game->ThrustLEFT = (TINYJOYPAD_LEFT);
  game->ThrustRIGHT = (TINYJOYPAD_RIGHT);
  ...
  for (y = 0; y < 8; y++)
  {
    ...
    for (x = 0; x < 128; x++)
    {
      if (mode == 0) {
        ...
        arduboy.SPItransfer ((GameDisplay(x, y, game) | LivesDisplay(x, y, game) | DashboardDisplay(x, y) | ScoreDisplay(x, y, score) | VelocityDisplay(x, y, velX, 1) | VelocityDisplay(x, y, velY, 0) | FuelDisplay(x, y, game)/*|BACKSCROLL(x,y,game)*/));
      } else if (mode == 1) {
        arduboy.SPItransfer ((pgm_read_byte(&INTRO[x + (y * 128)])));
      }
      else if (mode == 2)
      {
        arduboy.SPItransfer ((StarsDisplay ( x, y, game) | LivesDisplay(x, y, game) | DashboardDisplay(x, y) | ScoreDisplay(x, y, score) | VelocityDisplay(x, y, velX, 1) | VelocityDisplay(x, y, velY, 0) | FuelDisplay(x, y, game))/*|BACKSCROLL(x,y,game)*/);
      }
    }
    if (mode == 0 || mode == 2) {
    }
